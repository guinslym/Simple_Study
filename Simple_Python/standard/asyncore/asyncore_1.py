...
class EchoServer(asyncore.dispatcher):
    """Receive connections and establishes handlers for each client."""
    ...
    def handle_accept(self):
        ...
        EchoServer(sock=client_info[0])
        # Only deal with one client at a time
        # so close as soon as the handler is set up.
        # Under normal conditions,the server would run forever or until it received
...
class EchoHandler(asyncore.dispatcher):
    """Handlers echoing messages from a single client."""
    ...
    def writeable(self):
        ...
        self.logger.debug('writeable()->%s',response)
        return response
...
class EchoClient(asyncore.dispatcher):
    ...
    def handle_close(self):
        ...
        return
    def writeable(self):
        self.logger.debug('writeable()->%s',bool(self.to_send))
